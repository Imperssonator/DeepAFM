function ims = readNanoScopeImage(filePath)
...
if isempty(ind)
    error('Selected file is not a NanoScope AFM file.');
    fclose(fid);
    return
end
n = length(ind);
...
% Check for the height channel in the file
...
for k = 1:n
    % Read image data
    fseek(fid, data_offset(k), 'bof');
    im = rot90(fread(fid, [sizeX, sizeY], '*int16'));
    ims(k).img = im;
    ims(k).sizeX = sizeX;
    ims(k).sizeY = sizeY;
    ims(k).sizeX_nm = sizeX_nm;
    ims(k).sizeY_nm = sizeY_nm;
    ims(k).scaleXY = scaleXY;
    ims(k).scaleZ = scaleZ;
end
